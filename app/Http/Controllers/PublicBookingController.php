...
class PublicBookingController extends Controller
{
    public function show(Request $request, $businessSlug)
    {
        ...
        $selectedCategory = $request->input('category', 'featured');
        ...
        return view('public.business', compact('business', 'services', 'branches', 'teamMembers', 'bookingLimitReached', 'selectedCategory'));
    }

    /**
     * Multi-service slot availability endpoint.
     * GET /{slug}/slots?service_ids[]=1&service_ids[]=2&date=...&branch_id=...
     */
    public function multiServiceSlots(Request $request, $businessSlug)
    {
        $business = Business::where('slug', $businessSlug)->firstOrFail();
        ...
        // All submitted service_ids must be valid & belong to this business
        if ($services->count() !== count(array_unique(array_map('intval', $data['service_ids'])))) {
            return response()->json(['slots' => []]);
        }

        $totalDuration = (int) $services->sum('duration_minutes');

        return $this->calculateSlots($business, $data['date'], $data['branch_id'] ?? null, $totalDuration);
    }
    ...
    private function calculateSlots(Business $business, string $dateInput, ?int $branchId, int $durationMinutes)
    {
        $date = Carbon::parse($dateInput, $business->timezone)->startOfDay();
        ...
        if ($branchId) {
            $staffQuery->where(function ($q) use ($branchId) {
    ...
    public function store(Request $request, $businessSlug)
    {
        ...
        // Validate all selected services belong to this business
        ...
        // Verify slot falls within at least one defined shift
        $withinShift = false;
        $shifts = [[$wh->first_shift_start, $wh->first_shift_end]];
        if ($wh->second_shift_start && $wh->second_shift_end) {
        ...
        // Verify selected staff is valid
        ...
        if (!$selectedStaff) {
            return back()->withErrors(['staff_user_id' => 'Selected staff member is not available.'])->withInput();
        }
        ...
        if ($hasOverlap) {
            return back()->withErrors([
                'start_time' => 'Selected staff member is no longer available for this time. Please choose another slot.',
            ])->withInput();
        }
        ...
        // Notify company admins (skip if staff is also admin)
    ...
    public function success($businessSlug, $bookingId)
    {
        ...
        $booking  = Booking::where('business_id', $business->id)
            ->where('id', $bookingId)
            ->with(['service', 'services', 'branch', 'staff'])
            ->firstOrFail();
