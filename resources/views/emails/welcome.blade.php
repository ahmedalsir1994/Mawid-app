...
<div class="container">
    <div class="header">
        ...
        @if($plan !== 'free')
            <span class="badge">{{ ucfirst($plan) }} Plan Activated</span>
        @endif
    </div>
    ...
    <div class="body">
        ...
        <p class="message">
            @if($pendingPayment)
                Your email has been verified. You'll be redirected to complete your {{ ucfirst($plan) }} plan payment — once that's done, you'll have full access to everything.
            @elseif($plan !== 'free')
                Your payment was successful and your <strong>{{ ucfirst($plan) }} plan</strong> is now active. Your account is fully set up and ready to go!
            @else
        ...
        <div class="plan-box">
            ...
            <p class="plan-sub">
                @if($plan === 'free') 1 branch · 1 staff · up to 3 services
                @elseif($plan === 'pro') 1 branch · up to 3 staff · up to 15 services
                @else 3 branches · up to 15 staff · unlimited services
                @endif
            </p>
        </div>
        ...
        <a href="{{ route('admin.dashboard') }}" class="cta-btn">Go to Dashboard →</a>
        @endif

        <hr class="divider">
        <p class="note">
            If you didn't create this account, please ignore this email or
            <a href="mailto:{{ config('mail.from.address') }}" style="color:#16a34a;">contact support</a>.
        </p>
    </div>
