<x-admin-layout>
    ...
    <div class="mb-6 bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="font-semibold text-gray-800">{{ __('app.branches_plan_desc', ['used' => $branchUsed, 'max' => $branchMax]) }}</p>
                @if(!$isPlusOrAbove)
                    <p class="text-sm text-purple-600">
                        {{ __('app.upgrade_for_more_branches') }}
                        <a href="{{ route('admin.upgrade.index') }}" class="font-semibold underline hover:text-purple-800">{{ __('app.upgrade_plan') }}</a>
                    </p>
                @else
                    <p class="text-sm text-gray-500">{{ __('app.plus_branches_hint') }}</p>
                @endif
            </div>
        </div>
        <div class="w-48">
            <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div class="h-2 {{ $atLimit ? 'bg-red-500' : 'bg-green-500' }} rounded-full transition-all"
                     style="width: {{ min(100, ($branchUsed / max(1, $branchMax)) * 100) }}%"></div>
            </div>
            <p class="text-xs text-gray-400 mt-1 text-right">{{ $branchUsed }}/{{ $branchMax }}</p>
        </div>
    </div>
    ...
    @if($branches->isEmpty())
        <div class="bg-white rounded-xl shadow-md border border-gray-100 p-12 text-center">
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-2">{{ __('app.no_branches_yet') }}</h3>
            <p class="text-gray-500 mb-6">{{ __('app.create_first_branch') }}</p>
            <a href="{{ route('admin.branches.create') }}"
               class="inline-block px-6 py-3 rounded-lg bg-gradient-to-r from-purple-600 to-purple-500 text-white font-semibold hover:shadow-lg transition">
                {{ __('app.create_your_first_branch') }}
            </a>
        </div>
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach($branches as $branch)
                <div class="bg-white rounded-xl shadow-md border {{ $branch->is_main ? 'border-purple-300' : 'border-gray-100' }} overflow-hidden hover:shadow-lg transition">
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center gap-2">
                                <div class="w-10 h-10 rounded-lg {{ $branch->is_main ? 'bg-purple-100' : 'bg-gray-100' }} flex items-center justify-center">
                                    <svg class="w-5 h-5 {{ $branch->is_main ? 'text-purple-600' : 'text-gray-500' }}" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <div>
                                    ...
                                    @if($branch->is_main)
                                        <span class="text-xs font-semibold text-purple-600">{{ __('app.main_branch') }}</span>
                                    @endif
                                </div>
                            </div>
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold {{ $branch->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                        ...
                        <div class="flex items-center gap-4 text-sm text-gray-500 mb-4">
                            <span>{{ $branch->services_count }} {{ __('app.services') }}</span>
                            <span>·</span>
                            <span>{{ $branch->staff_count }} {{ __('app.staff') }}</span>
                        </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(!$branch->is_main)
                                <form method="POST" action="{{ route('admin.branches.destroy', $branch) }}" onsubmit="return confirm('{{ __('app.confirm_delete_branch') }}')">
                                    @csrf @method('DELETE')
