    ...
    <div id="invoice-paper" class="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-10">
        ...
        <div class="flex justify-between items-start mb-10">
            ...
            <div class="text-right">
                ...
                <p class="text-lg font-mono font-semibold text-gray-600 mt-1">{{ $invoice->invoice_number }}</p>
                @php
                ...
                @endphp
                <span class="inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold {{ $statusClass }}">
        ...
        <div class="grid grid-cols-2 gap-8 mb-10">
            <div>
                ...
                <p class="font-bold text-gray-900 text-lg">{{ $invoice->business_name }}</p>
                @if($invoice->business_email)
                    <p class="text-gray-500 text-sm">{{ $invoice->business_email }}</p>
                @endif
            </div>
            <div class="text-right">
                ...
                <div class="space-y-1 text-sm">
                    @if($invoice->paid_at)
                        <div class="flex justify-end gap-4">
                            ...
                            <span class="font-medium text-gray-800">{{ $invoice->paid_at->format('d M Y') }}</span>
                        </div>
                    @endif
                    ...
                    @if($invoice->paymob_transaction_id)
                        <div class="flex justify-end gap-4">
                            ...
                            <span class="font-mono text-xs text-gray-600">{{ $invoice->paymob_transaction_id }}</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <table class="w-full mb-8 text-sm">
            ...
            <tbody>
                @php $planEmoji = match($invoice->plan) { 'pro' => '💼', 'plus' => '🚀', default => '🆓' }; @endphp
                <tr class="border-b border-gray-100">
                    <td class="py-4">
                        <p class="font-semibold text-gray-800">
                            {{ $planEmoji }} Mawid {{ ucfirst($invoice->plan) }} Plan
                        </p>
                        @if($invoice->billing_period_start && $invoice->billing_period_end)
                    ...
                    <td class="py-4 text-center text-gray-600">{{ ucfirst($invoice->billing_cycle ?? '—') }}</td>
